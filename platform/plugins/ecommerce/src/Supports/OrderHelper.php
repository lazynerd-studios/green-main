...
class OrderHelper
{
        ...
        foreach ($orders as $order) {
            app(OrderHistoryInterface::class)->createOrUpdate([
                ...
                'description' => trans('plugins/ecommerce::order.new_order_from', [
                    ...
                    'customer' => clean($order->user->name ?: $order->address->name),
                ]),
                'order_id'    => $order->id,
            ]);
        }
    ...
    public function setEmailVariables($order)
    {
        return EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME)
            ->setVariableValues([
                ...
                'customer_name'        => clean($order->user->name ?: $order->address->name),
    ...
    public function downloadInvoice($order)
    {
        ...
        $invoice = PDF::loadView($template, compact('order'), [], 'UTF-8');
        ...
        return $invoice
            ...
            ->download('invoice-order-' . get_order_code($order->id) . '.pdf');
    }
    ...
    public function processHistoryVariables($history)
    {
        ...
        $variables = [
            ...
            'user_name' => $history->user_id === 0 ? trans('plugins/ecommerce::order.system') :
                clean($history->user ? $history->user->name : ($history->order->user->name ?:
