                                ...
                                WithChunkReading
{
    ...
    public function storeProduction()
    {
        ...
        $storeAttr->execute($product, $attributeSets);
        $collect = collect([
            'name'           => $product->name,
            'import_type'    => 'product',
    ...
    public function storeVariant($product)
    {
        ...
        if (!$result['created']) {
            if (method_exists($this, 'onFailure')) {
                $failures[] = new Failure(
                    $this->rowCurrent,
                    'variation',
                    [trans('plugins/ecommerce::products.form.variation_existed') . __(' Product Id: ') . $product->id],
                    []
                );
                $this->onFailure(...$failures);
            }
        ...
        if (!$variation->product_id || $isUpdateProduct) {
            ...
            $images = array_values(array_filter(Arr::get($version, 'images') ?: []));
